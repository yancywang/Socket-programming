...
#include <sys/wait.h>
#include <signal.h>
#define PORT "23000209"  // the port users will be connecting to
#define BACKLOG 10   // how many pending connections queue will hold
#define FILENAME "serverC.txt" //the input server file
...
{   
    ...
    while(!feof(f))
    {
        ...
        if(strncmp(buf[index],"serverA",7)==0)
        {
            matrix[2][0]=cost[index];
            matrix[0][2]=cost[index];
        }
        else if(strncmp(buf[index],"serverB",7)==0)
        {
            matrix[2][1]=cost[index];
            matrix[1][2]=cost[index];
        }
        else if(strncmp(buf[index],"serverD",7)==0)
        {
